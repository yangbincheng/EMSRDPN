...
class RCAN(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        ...
        self.upsample = nn.ModuleList([
            common.Upsampler(common.default_conv, s, n_feats, act=False) for s in args.scale
        ])

        self.tail = nn.ModuleList([
            common.default_conv(n_feats, args.n_colors, kernel_size) for s in args.scale
        ])
    ...
    def forward(self, x):
        ...
        if self.multi_scale_infer:
            ...
            for s in range(len(self.scale)):
                ...
                x = self.tail[s](x)
                results.append(self.add_mean(x))
            return results
